...
(function ($) {
   ...
   */
  var _p = $plugins.filter(function (p) {
    return p.description.contains('<Iavra Localization Core>');
  })[0].parameters;
  var _param_escape = _p['Escape Code'];
  var _param_languages = _p['Languages'].split(/\s*,\s*/).filter(function (lang) {
    return !!lang;
  });
   ...
   */
  var _data = _param_languages.reduce(function (map, lang) {
    map[lang] = null;
    return map;
  }, Object.create(null));
  ...
  var _initialize = function () {
    _param_languages.forEach(function (lang) {
      _loadFile(_param_filePath.replace('{lang}', lang), function (data) {
        _data[lang] = _flatten(data);
      });
    });
    _data[undefined] = {};
  };
  ...
  var _isReady = function () {
    return _param_languages.every(function (lang) {
      return !!_data[lang];
    });
  };
   ...
   */
  var _replace = function (text, ...params) {
    ...
    while (f) {
      f = false;
      r = r.replace(_regex, function (m, k) {
        f = true;
        return d[k];
      });
    }
    if (params && params.length) r = r.replace(/{(.*?)}/g, (match, idx) => params[idx]);
    return r;
  };
  ...
  (function ($) {
    ...
    $.prototype.isReady = function () {
      return _isReady() && alias_isReady.call(this);
    };
    ...
    $.prototype.updateDocumentTitle = function () {
      if ($dataSystem) {
        document.title = _replace($dataSystem.gameTitle);
      }
    };
  })(Scene_Boot);
  ...
  (function ($) {
    ...
    $.prototype.drawText = function (text, x, y, maxWidth, lineHeight, align) {
      alias_drawText.call(
        this,
        _inDrawTextEx ? text : _replace(text),
        x,
        y,
        maxWidth,
        lineHeight,
